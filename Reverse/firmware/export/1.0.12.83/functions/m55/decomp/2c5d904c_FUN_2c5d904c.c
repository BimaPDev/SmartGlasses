/* FUN_2c5d904c @ 0x2c5d904c */

void FUN_2c5d904c(int param_1,undefined4 *param_2,int param_3,int param_4)

{
  undefined4 uVar1;
  undefined4 *puVar2;
  undefined4 uVar3;
  undefined4 local_20;
  int local_1c;
  
  local_1c = *DAT_2c5d917c;
  uVar1 = FUN_2c637344(*(undefined4 *)(param_1 + 4),param_2,param_3,0);
  *(undefined4 *)(param_1 + 0x10) = uVar1;
  FUN_2c638730(uVar1,*param_2);
  FUN_2c606e20(*(undefined4 *)(param_1 + 0x10),0xff00ff00,0);
  uVar3 = *(undefined4 *)(param_1 + 0x10);
  uVar1 = FUN_2c5e2e80(DAT_2c5d9180);
  FUN_2c606e38(uVar3,uVar1,0);
  FUN_2c607048(*(undefined4 *)(param_1 + 0x10),0x27d1);
  FUN_2c6072bc(*(undefined4 *)(param_1 + 0x10),2,0,10);
  if (param_3 != 0) {
    FUN_2c602454(*(undefined4 *)(param_1 + 0x10),DAT_2c5d9188,*DAT_2c5d9184,0);
    local_20 = FUN_2c620f3c(param_3,*(undefined4 *)(param_1 + 0x10),0);
    puVar2 = *(undefined4 **)(param_1 + 0x38);
    if (puVar2 == *(undefined4 **)(param_1 + 0x3c)) {
      FUN_2c5d8c2c(param_1 + 0x34,puVar2,&local_20);
    }
    else {
      *puVar2 = local_20;
      *(int *)(param_1 + 0x38) = *(int *)(param_1 + 0x38) + 4;
    }
  }
  uVar1 = FUN_2c637344(*(undefined4 *)(param_1 + 4));
  *(undefined4 *)(param_1 + 0x14) = uVar1;
  FUN_2c638730(uVar1,*param_2);
  FUN_2c606e20(*(undefined4 *)(param_1 + 0x14),0xff00ff00,0);
  uVar3 = *(undefined4 *)(param_1 + 0x14);
  uVar1 = FUN_2c5e2e80(DAT_2c5d9180);
  FUN_2c606e38(uVar3,uVar1,0);
  FUN_2c607048(*(undefined4 *)(param_1 + 0x14),0x27d1);
  FUN_2c6072bc(*(undefined4 *)(param_1 + 0x14),5,0,0xfffffff8);
  if (param_4 != 0) {
    FUN_2c602454(*(undefined4 *)(param_1 + 0x14),DAT_2c5d918c,*DAT_2c5d9184,0);
    local_20 = FUN_2c620f3c(param_4,*(undefined4 *)(param_1 + 0x14),0);
    puVar2 = *(undefined4 **)(param_1 + 0x38);
    if (puVar2 == *(undefined4 **)(param_1 + 0x3c)) {
      FUN_2c5d8c2c(param_1 + 0x34,puVar2,&local_20);
    }
    else {
      *puVar2 = local_20;
      *(int *)(param_1 + 0x38) = *(int *)(param_1 + 0x38) + 4;
    }
  }
  if (*DAT_2c5d917c == local_1c) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  stack_chk_fail();
}

