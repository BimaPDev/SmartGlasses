...
class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
    ...
    void different(MotionConstrainedPoint points, boolean[] mask, String[] custom) {
        ...
        mask[1] = mask[1] | diff(this.f53x, points.f53x);
        mask[2] = mask[2] | diff(this.f54y, points.f54y);
    ...
    void fillStandard(double[] data, int[] toUse) {
        float[] fArr = {this.position, this.f53x, this.f54y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        int i = 0;
        for (int i2 : toUse) {
            if (i2 < 18) {
                data[i] = fArr[i2];
