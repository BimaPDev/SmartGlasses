...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f8 = startTimePoint.f57x;
        float f9 = startTimePoint.f58y;
        float f10 = (endTimePoint.f57x + (f4 / 2.0f)) - ((f5 / 2.0f) + f8);
        float f11 = (endTimePoint.f58y + (f6 / 2.0f)) - (f9 + (f7 / 2.0f));
        float f12 = (f4 - f5) * f2;
        float f13 = f12 / 2.0f;
        this.f57x = (int) ((f8 + (f10 * f)) - f13);
        float f14 = (f6 - f7) * f3;
        float f15 = f14 / 2.0f;
        this.f58y = (int) ((f9 + (f11 * f)) - f15);
        ...
        this.mMode = 0;
        this.f57x = (int) (((startTimePoint.f57x + (f16 * f10)) + (f18 * f11)) - f13);
        this.f58y = (int) (((startTimePoint.f58y + (f10 * f17)) + (f11 * f)) - f15);
    ...
    void initPolar(int parentWidth, int parentHeight, KeyPosition c, MotionPaths s, MotionPaths e) {
        ...
        if (i == 1) {
            ...
            float f10 = e.f57x;
            float f11 = s.f57x;
            this.f57x = (f9 * (f10 - f11)) + f11;
            if (!Float.isNaN(c.mPercentY)) {
                f2 = c.mPercentY;
            }
            float f12 = e.f58y;
            float f13 = s.f58y;
            this.f58y = (f2 * (f12 - f13)) + f13;
        } else if (i == 2) {
            ...
            float f19 = e.f57x;
            float f20 = s.f57x;
            this.f57x = (f18 * (f19 - f20)) + f20;
            if (!Float.isNaN(c.mPercentY)) {
                f2 = c.mPercentY;
            }
            float f21 = e.f58y;
            float f22 = s.f58y;
            this.f58y = (f2 * (f21 - f22)) + f22;
        }
    ...
    public void setupRelative(MotionController mc, MotionPaths relative) {
        double d = ((this.f57x + (this.width / 2.0f)) - relative.f57x) - (relative.width / 2.0f);
        double d2 = ((this.f58y + (this.height / 2.0f)) - relative.f58y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f57x = (float) Math.hypot(d2, d);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f58y = (float) (Math.atan2(d2, d) + 1.5707963267948966d);
        } else {
            this.f58y = (float) Math.toRadians(this.mRelativeAngle);
        }
    }
    ...
    void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f8 = startTimePoint.f57x;
        float f9 = startTimePoint.f58y;
        float f10 = endTimePoint.f57x + (f4 / 2.0f);
        float f11 = endTimePoint.f58y + (f6 / 2.0f);
        float f12 = (f4 - f5) * f2;
        this.f57x = (int) ((f8 + ((f10 - ((f5 / 2.0f) + f8)) * f)) - (f12 / 2.0f));
        float f13 = (f6 - f7) * f3;
        this.f58y = (int) ((f9 + ((f11 - (f9 + (f7 / 2.0f))) * f)) - (f13 / 2.0f));
        this.width = (int) (f5 + f12);
        this.height = (int) (f7 + f13);
        this.mMode = 2;
        if (!Float.isNaN(c.mPercentX)) {
            this.f57x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f58y = (int) (c.mPercentY * ((int) (parentHeight - this.height)));
        }
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f6 = startTimePoint.f57x;
        float f7 = startTimePoint.width;
        float f8 = startTimePoint.f58y;
        float f9 = startTimePoint.height;
        float f10 = (endTimePoint.f57x + (endTimePoint.width / 2.0f)) - ((f7 / 2.0f) + f6);
        float f11 = (endTimePoint.f58y + (endTimePoint.height / 2.0f)) - ((f9 / 2.0f) + f8);
        float f12 = f10 * f;
        float f13 = f4 * f2;
        float f14 = f13 / 2.0f;
        this.f57x = (int) ((f6 + f12) - f14);
        float f15 = f * f11;
        float f16 = f5 * f3;
        float f17 = f16 / 2.0f;
        this.f58y = (int) ((f8 + f15) - f17);
        this.width = (int) (f7 + f13);
        this.height = (int) (f9 + f16);
        float f18 = Float.isNaN(c.mPercentY) ? 0.0f : c.mPercentY;
        this.mMode = 1;
        float f19 = (int) ((startTimePoint.f57x + f12) - f14);
        this.f57x = f19;
        float f20 = (int) ((startTimePoint.f58y + f15) - f17);
        this.f57x = f19 + ((-f11) * f18);
        this.f58y = f20 + (f10 * f18);
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean zDiff = diff(this.f57x, points.f57x);
        boolean zDiff2 = diff(this.f58y, points.f58y);
    ...
    void getCenter(double p, int[] toUse, double[] data, float[] point, int offset) {
        float fSin = this.f57x;
        float fCos = this.f58y;
        float f = this.width;
    ...
    void fillStandard(double[] data, int[] toUse) {
        float[] fArr = {this.position, this.f57x, this.f58y, this.width, this.height, this.mPathRotate};
        int i = 0;
        for (int i2 : toUse) {
            if (i2 < 6) {
                data[i] = fArr[i2];
