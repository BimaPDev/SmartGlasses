...
public final class TaskLoggerKt {
    ...
    public static final <T> T logElapsed(Task task, TaskQueue queue, Function0<? extends T> block) {
        ...
        try {
            ...
            if (zIsLoggable) {
                String strStringPlus = Intrinsics.stringPlus("finished run in ", formatDuration(queue.getTaskRunner().getBackend().nanoTime() - jNanoTime));
            }
            return tInvoke;
        } finally {
            InlineMarker.finallyStart(1);
            if (zIsLoggable) {
                log(task, queue, Intrinsics.stringPlus("failed a run in ", formatDuration(queue.getTaskRunner().getBackend().nanoTime() - jNanoTime)));
            }
    ...
    public static final String formatDuration(long j) {
        ...
        if (j <= -999500000) {
            ...
            str = ((j - ((long) JsonLocation.MAX_CONTENT_SNIPPET)) / ((long) 1000)) + " µs";
        } else if (j < 999500) {
            str = ((j + ((long) JsonLocation.MAX_CONTENT_SNIPPET)) / ((long) 1000)) + " µs";
        } else if (j < 999500000) {
            str = ((j + ((long) 500000)) / ((long) DurationKt.NANOS_IN_MILLIS)) + " ms";
