...
public abstract class SettableBeanProperty extends ConcreteBeanPropertyBase implements Serializable {
    ...
    protected void _throwAsIOE(JsonParser jsonParser, Exception exc, Object obj) throws IOException {
        if (exc instanceof IllegalArgumentException) {
            StringBuilder sbAppend = new StringBuilder("Problem deserializing property '").append(getName()).append("' (expected type: ").append(getType()).append("; actual type: ").append(ClassUtil.classNameOf(obj)).append(")");
            String strExceptionMessage = ClassUtil.exceptionMessage(exc);
            if (strExceptionMessage != null) {
                sbAppend.append(", problem: ").append(strExceptionMessage);
            } else {
                sbAppend.append(" (no error message provided)");
            }
            throw JsonMappingException.from(jsonParser, sbAppend.toString(), exc);
        }
