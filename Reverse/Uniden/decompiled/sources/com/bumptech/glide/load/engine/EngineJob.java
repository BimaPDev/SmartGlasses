...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallResourceReady(resourceCallbackAndExecutor.f171cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallLoadFailed(resourceCallbackAndExecutor.f171cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        private final ResourceCallback f169cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f169cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f169cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f169cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f169cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* JADX INFO: renamed from: cb */
        private final ResourceCallback f170cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f170cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f170cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f170cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f170cb);
                        EngineJob.this.removeCallback(this.f170cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        /* JADX INFO: renamed from: cb */
        final ResourceCallback f171cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f171cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f171cb.equals(((ResourceCallbackAndExecutor) obj).f171cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f171cb.hashCode();
        }
    }
