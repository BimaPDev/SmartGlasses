...
public class SessionReportingCoordinator {
    ...
    public void persistFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m458v("Persisting fatal event for session " + str);
        persistEvent(th, thread, str, "crash", j, true);
    }

    public void persistNonFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m458v("Persisting non-fatal event for session " + str);
    ...
    public void persistRelevantAppExitInfoEvent(String str, List<ApplicationExitInfo> list, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (applicationExitInfoFindRelevantApplicationExitInfo == null) {
            Logger.getLogger().m458v("No relevant ApplicationExitInfo occurred during session: " + str);
            return;
        }
        ...
        Logger.getLogger().m452d("Persisting anr for session " + str);
    ...
    public void finalizeSessionWithNativeEvent(String str, List<NativeSessionFile> list, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        Logger.getLogger().m452d("SessionReportingCoordinator#finalizeSessionWithNativeEvent");
    ...
    private CrashlyticsReport.Session.Event addLogsAndCustomKeysToEvent(CrashlyticsReport.Session.Event event, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().m458v("No log data to include with this event.");
        }
        ...
        if (!z) {
            this.crashlyticsWorkers.diskWrite.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.SessionReportingCoordinator$$ExternalSyntheticLambda0
                ...
                public final void run() {
                    this.f$0.m474x631b9fbf(eventAddMetaDataToEvent, str, zEquals);
                }
            });
        } else {
    ...
    /* JADX INFO: renamed from: lambda$persistEvent$0$com-google-firebase-crashlytics-internal-common-SessionReportingCoordinator */
    /* synthetic */ void m474x631b9fbf(CrashlyticsReport.Session.Event event, String str, boolean z) {
        Logger.getLogger().m452d("disk worker: log non-fatal event to persistence");
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger.getLogger().m452d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            File reportFile = result.getReportFile();
            if (reportFile.delete()) {
                Logger.getLogger().m452d("Deleted report file: " + reportFile.getPath());
                return true;
            }
            Logger.getLogger().m460w("Crashlytics could not delete report file: " + reportFile.getPath());
            return true;
        }
        Logger.getLogger().m461w("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().m460w("Could not get input trace in application exit info: " + applicationExitInfo.toString() + " Error: " + e);
        }
