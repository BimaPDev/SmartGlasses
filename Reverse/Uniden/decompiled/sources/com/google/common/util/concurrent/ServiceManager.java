...
public final class ServiceManager implements ServiceManagerBridge {
    ...
    public ServiceManager(Iterable<? extends Service> iterable) {
        ...
        if (immutableListCopyOf.isEmpty()) {
            ...
            immutableListCopyOf = ImmutableList.m276of(new NoOpService());
        }
        ...
        void awaitHealthy(long j, TimeUnit timeUnit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.awaitHealthGuard, j, timeUnit)) {
                    String strValueOf = String.valueOf(Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.m249in(ImmutableSet.m324of(Service.State.NEW, Service.State.STARTING))));
        ...
        void awaitStopped(long j, TimeUnit timeUnit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.stoppedGuard, j, timeUnit)) {
                    String strValueOf = String.valueOf(Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.not(Predicates.m249in(EnumSet.of(Service.State.TERMINATED, Service.State.FAILED)))));
