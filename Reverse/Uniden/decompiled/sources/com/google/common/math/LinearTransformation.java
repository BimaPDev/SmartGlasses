...
public abstract class LinearTransformation {
    ...
    public static final class LinearTransformationBuilder {
        ...
        private final double f365x1;

        /* JADX INFO: renamed from: y1 */
        private final double f366y1;
        ...
        public LinearTransformation and(double d, double d2) {
            Preconditions.checkArgument(DoubleUtils.isFinite(d) && DoubleUtils.isFinite(d2));
            double d3 = this.f365x1;
            if (d == d3) {
                Preconditions.checkArgument(d2 != this.f366y1);
                return new VerticalLinearTransformation(this.f365x1);
            }
            return withSlope((d2 - this.f366y1) / (d - d3));
        }

        public LinearTransformation withSlope(double d) {
            Preconditions.checkArgument(!Double.isNaN(d));
            if (DoubleUtils.isFinite(d)) {
                return new RegularLinearTransformation(d, this.f366y1 - (this.f365x1 * d));
            }
            return new VerticalLinearTransformation(this.f365x1);
        }
    }
    ...
    private static final class VerticalLinearTransformation extends LinearTransformation {
        ...
        public String toString() {
            return String.format("x = %g", Double.valueOf(this.f367x));
        }

        private LinearTransformation createInverse() {
            return new RegularLinearTransformation(0.0d, this.f367x, this);
        }
    }
