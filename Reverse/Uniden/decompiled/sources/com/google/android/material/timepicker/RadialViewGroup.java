...
class RadialViewGroup extends ConstraintLayout {
    ...
    public RadialViewGroup(Context context, AttributeSet attributeSet, int i) {
        ...
        LayoutInflater.from(context).inflate(C1081R.layout.material_radial_view_group, this);
        ViewCompat.setBackground(this, createBackground());
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C1081R.styleable.RadialViewGroup, i, 0);
        this.radius = typedArrayObtainStyledAttributes.getDimensionPixelSize(C1081R.styleable.RadialViewGroup_materialCircleRadius, 0);
    ...
    protected void updateLayoutParams() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (childAt.getId() != C1081R.id.circle_center && !shouldSkipView(childAt)) {
                int i2 = (Integer) childAt.getTag(C1081R.id.material_clock_level);
                if (i2 == null) {
                    i2 = 1;
                }
    ...
    private void addConstraints(List<View> list, ConstraintSet constraintSet, int i) {
        ...
        float size = 0.0f;
        while (it.hasNext()) {
            constraintSet.constrainCircle(it.next().getId(), C1081R.id.circle_center, i, size);
